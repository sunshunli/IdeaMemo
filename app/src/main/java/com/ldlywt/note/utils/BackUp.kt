...
object BackUp {
    ...
    suspend fun exportEncrypted(context: Context, uri: Uri): String =
        suspendCoroutine { continuation ->
            context.contentResolver.openOutputStream(uri)?.use { stream ->
                ...
                val zipStream =
                    ZipOutputStream(BufferedOutputStream(CipherOutputStream(stream, cipher)))
                try {
                    val files = listOf(
    ...
    suspend fun restoreFromEncryptedZip(context: Context, uri: Uri, isEncrypt: Boolean) =
        withContext(Dispatchers.IO) {
            ...
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                ...
                if (File(destFile.path + "/databases").exists()) {
                    // restore database
                    File(destFile.path + "/databases").copyRecursively(
                        File(context.dataDir.path + "/databases"),
                        true
                    )
                }
                ...
                if (File(destFile.path + "/external/files").exists()) {
                    ...
                    File(destFile.path + "/external/files").copyRecursively(
    ...
    suspend fun exportJson(list: List<NoteShowBean>, uri: Uri): Result<Unit> =
        withContext(Dispatchers.IO) {
